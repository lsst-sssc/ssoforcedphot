...
from forcedphot.ephemeris.local_dataclasses import QueryInput, QueryResult, EphemerisData
...
def test_query_from_csv_horizons(mock_query_csv, ephemeris_client):
    ...
    mock_query_csv.return_value = [
        QueryResult(target="Ceres", start=Time("2023-01-01"), end=Time("2023-01-02"), ephemeris=EphemerisData()),
        QueryResult(target="Vesta", start=Time("2023-01-01"), end=Time("2023-01-02"), ephemeris=EphemerisData())
    ]
...
def test_query_from_csv_miriade(mock_query_csv, ephemeris_client):
    ...
    """
    mock_query_csv.return_value = [
        QueryResult(target="Encke", start=Time("2023-01-01"), end=Time("2023-01-02"), ephemeris=EphemerisData()),
        QueryResult(target="Halley", start=Time("2023-01-01"), end=Time("2023-01-02"), ephemeris=EphemerisData())
    ]
