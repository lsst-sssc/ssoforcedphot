    ...
    def search_images_polygon(
        ...
        for i, item in enumerate(polygons):
            try:
                ...
                self.logger.debug(
                    f"""Polygon {i+1}: Querying with timespan {timespan_search}
                    and region from {len(corners)} corners."""
                )
        ...
        for ref_idx, ref in enumerate(unique_refs):
            try:
                ...
                if ephemeris:
                    ...
                    self.logger.debug(
                        f"""Ref {ref.dataId}: Found {len(relevant_eph_for_image)}
                        relevant ephem rows for exposure {t_min.isot} - {t_max.isot}"""
                    )
                    ...
                    if relevant_eph_for_image:
                        ...
                        if len(relevant_eph_for_image) == 1:
                            ...
                            self.logger.debug(
                                f"""Ref {ref.dataId}: Interpolated exact_eph:
                                {exact_eph_for_image}, aux from: {closest_eph_for_aux_data.datetime.isot}"""
                            )
                    else:
                        self.logger.warning(
                            f"Ref {ref.dataId}: No relevant ephemeris data found {t_min.isot} - {t_max.isot}."
                        )
                else:
                    self.logger.info(
                        "No global ephemeris data provided. Skipping ephemeris processing for images."
                    )
        ...
        self.logger.info(
            f"Created {len(image_metadata_list)} ImageMetadata objects before final deduplication."
        )
