...
class EphemerisData:
    ...
    @staticmethod
    def load_ephemeris(file_path: str) -> List['EphemerisData']:
        """
        Loads ephemeris data from an ECSV file and converts it to EphemerisData objects.

        Args:
            file_path (str): Path to the ECSV file containing ephemeris data.

        Returns:
            List[EphemerisData]: List of parsed ephemeris data rows.
    ...
    @staticmethod
    def get_relevant_rows(ephemeris_rows: List['EphemerisData'], t_min: Time, t_max: Time) -> List['EphemerisData']:
        """
        Retrieves 2-3 rows of ephemeris data suitable for interpolation within a time range.

        Args:
            ephemeris_rows (List[EphemerisData]): Complete list of ephemeris rows.
            t_min (Time): Start time of the desired range.
            t_max (Time): End time of the desired range.

        Returns:
            List[EphemerisData]: 2-3 rows containing the relevant time period and adjacent data points.
        """
    ...
    @staticmethod
    def get_time_range(rows: List['EphemerisData']) -> Tuple[Time, Time]:
        """
        Determines the complete time span covered by the ephemeris data.

        Args:
            rows (List[EphemerisData]): List of ephemeris rows.
    ...
    @staticmethod
    def get_time_windows(rows: List['EphemerisData']) -> List[Tuple[Time, Time, float, float]]:
        """
...
class ImageMetadata:
    ...
    ephemeris_data : List[EphemerisData]
        List of ephemeris data
    exact_ephemeris : EphemerisData
