...
from forcedphot.image_photometry.utils import ErrorEllipse, PhotometryResult, EndResult, ImageMetadata
...
class PhotometryService:
    ...
    def process_image(self, image_metadata: ImageMetadata, target_name: str, 
        ...
        print("-" * 70)
        print("Photometry Service is starting.")
        print("-" * 70)
        
        if image_metadata is None:
            return print("Image metadata is None")
        ...
        if len(found_sources["coord_ra"]) > 1:
            for i in range(1, len(found_sources)):
                source_result = PhotometryResult(
                    ...
                    ra_err=float(found_sources[i].get('coord_raErr')) if found_sources[i].get('coord_raErr') > 0 else 0,
                    dec_err=float(found_sources[i].get('coord_decErr')) if found_sources[i].get('coord_decErr') > 0 else 0,
                    x=found_sources[i].get('slot_Centroid_x'),
                    y=found_sources[i].get('slot_Centroid_y'),
                    x_err=float(found_sources[i].get('slot_Centroid_xErr')) if found_sources[i].get('slot_Centroid_xErr') >  0 else 0,
                    y_err=float(found_sources[i].get('slot_Centroid_yErr')) if found_sources[i].get('slot_Centroid_yErr') > 0 else 0,
                    ...
                    flux=found_sources[i].get('base_PsfFlux_instFlux') if found_sources[i].get('base_PsfFlux_instFlux') > 0 else 0,
                    flux_err=found_sources[i].get('base_PsfFlux_instFluxErr') if found_sources[i].get('base_PsfFlux_instFluxErr') > 0 else 0,
                    mag=-2.5 * np.log10(found_sources[i].get('base_PsfFlux_instFlux')) + 31.4 if found_sources[i].get('base_PsfFlux_instFlux') > 0 else 0,
                    ...
                    flags={
                        ...
                        'base_PsfFlux_flag_badCentroid_edge' : found_sources[0].get('base_PsfFlux_flag_badCentroid_edge'),
    ...
    def _prepare_end_results(self, target_name, target_type, ephemeris_service, image_metadata, cutout_size, saved_image_name, target_result, sources_within_error):
        """
